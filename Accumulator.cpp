...
//	pid->resetPIDAngle(); //if loop is done reset values
	...
	driveX = fabs(driveX) > .8 ? std::copysign(.8, driveX) : driveX; //if angleOutput is above max, set to max
	driveY = fabs(driveY) > .8 ? std::copysign(.8, driveY) : driveY; //if angleOutput is above max, set to max
