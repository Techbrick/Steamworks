...
//		}
		...
		if (gearOpenTimer.Get() > 10) { //if it's been open for more than 10 seconds
			...
			gear.setPusher(!gear.getPusher()); //TODO: may need to flip
			gearOpenTimer.Reset();
		}

		//gear.getBottom = true - open
		//gear.getBottom = false - closed
...
void Robot::Autonomous() {
	...
	case 1://Position 1: straight from the middle peg
		...
		while ((leftUltrasonic != 0 || rightUltrasonic != 0) && failsafe < 700 && !IsOperatorControl() && IsEnabled()) {//This makes sure that one of the ultrasonics is working
			driveVals = accum.drive(started, true, false, false, 8, desiredAngle, 2, gyro.GetYaw());//This is under the impression that the encoders are moving positive and the motors but the robot distance from the peg is decreasing
			started = true;
			driveX = driveVals.x;
			driveY = driveVals.y;
			driveZ = driveVals.angle;
	...
	case 2://Position 2: on the left
		...
		while (failsafe < 700 && !IsOperatorControl() && IsEnabled()) {//Drive out
			driveVals = accum.drive(started, true, false, false, 6, desiredAngle, 2, gyro.GetYaw());
			started = true;
			driveX = driveVals.x;
			driveY = driveVals.y;
			driveZ = driveVals.angle;
	...
	case 3://Position 3: on the right
		...
		while (failsafe < 700 && !IsOperatorControl() && IsEnabled()) {
			driveVals = accum.drive(started, true, false, false, 4, desiredAngle, 2, gyro.GetYaw());
			started = true;
			driveX = driveVals.x;
			driveY = driveVals.y;
			driveZ = driveVals.angle;
